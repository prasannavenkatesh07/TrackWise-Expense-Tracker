 ...
 *
 * Per-category monthly budget goal tracker.
 *
 * Features:
 *   - Month/year navigator so you can review past months
 *   - Health strip showing total allocated, spent, remaining, and over-budget count
 *   - Budget card grid - each card has an animated progress bar, edit, and delete
 *   - Add/Edit modal (POSTs to /api/budgets - backend handles create-or-update)
 *   - Warning banner when some categories still have no limit set
 *
 * API calls:
 *   GET    /api/budgets?month=&year=         → budget list with actual spend merged in
 *   GET    /api/budgets/summary?month=&year= → health strip totals
 *   POST   /api/budgets                      → create or update a budget
 *   DELETE /api/budgets/:id                  → remove one
 *
 * Data flow:
 *   BudgetsPage mounts → GET /api/budgets → Budget.getBudgetsWithSpend() (model static)
 *   joins budgets + Transaction aggregates → merged JSON → React state → progress bars
 */
...
// Same colours as ExpenseChart.jsx - kept in sync manually
// TODO: extract this to a shared constants file so it only lives in one place
const CAT_COLOR = {
  'Housing':          '#6366f1',
...
// Formatting helper - consistent across this file
const fmtINR = (n) => `₹${(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}`;

// --- Custom select ------------------------------------------------------------
// Native <select> can't be styled consistently across browsers, so using a
// custom dropdown - same pattern as TransactionForm and TransactionTable
...
// --- Health pill --------------------------------------------------------------
// Used in the header strip - shows totals at a glance
const HealthPill = ({ label, value, accent, isLoading }) => (
  <div className="px-3 sm:px-4 py-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-card text-center overflow-hidden">
    ...
    {isLoading
      ...
      : <p className={`font-numeric text-sm sm:text-base font-bold truncate ${accent}`}>{value}</p>
    }
  </div>
);

// --- Budget card --------------------------------------------------------------
// Progress bar colour thresholds: green < 50%, amber 50–79%, red 80%+
const BudgetCard = ({ budget, onEdit, onDelete, deleting }) => {
  ...
  return (
    <article className="card hover:scale-[1.01] transition-transform duration-200 relative group">
      ...
      {/* Delete - only visible on hover so it doesn't clutter the card */}
      <button
      ...
      <div className="flex items-start gap-3 mb-4">
        <div
          ...
          style={{ backgroundColor: `${color}20` }} // 20 = 12.5% opacity hex
          aria-hidden="true"
        >
        ...
        <div className="min-w-0 pr-6 flex-1">
          <div className="flex items-center gap-2">
            ...
            {isOverBudget && (
              <span
                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-rose-100 dark:bg-rose-900/30 flex-shrink-0"
                title="This category has exceeded its limit"
              >
      ...
      {/* Progress bar - CSS variable width for the same JIT reason as the dashboard */}
      <div
...
// --- Add / Edit modal ---------------------------------------------------------
// Same modal handles both create and edit - the `initial` prop determines the mode.
// POST /api/budgets handles both (backend uses findOneAndUpdate with upsert).
const BudgetModal = ({ initial, month, year, usedCategories, onSave, onClose }) => {
  ...
  // When editing, show all categories (including the one already selected).
  // When adding, only show categories that don't have a budget yet this month.
        ...
        <div className="space-y-4">
          <div className="relative z-20">
            ...
            <CustomSelect
              ...
              disabled={isEdit} // can't change category when editing - it's the unique key
              error={err.category}
            />
          ...
          <div className="relative z-10">
            ...
            <input
              id="bm-limit" type="number" min="1" value={limit}
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={handleSave} disabled={saving} className="btn-primary flex-1 py-2.5">
            {saving
              ...
              : <><CheckCircle2 size={14} />{isEdit ? 'Update' : 'Add'} budget</>
            }
          </button>
        </div>
      </div>
...
const BudgetsPage = () => {
  ...
  // Fetch budgets and health summary in parallel - no reason to wait for one before the other
  ...
  const nextMonth = () => {
    ...
    if (year > nowY || (year === nowY && month >= nowM)) return; // don't go into the future
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchData(); // re-fetch to update the health summary strip
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <section aria-label="Budget health summary" className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <HealthPill
          label="Over budget"
          value={`${summary?.overBudgetCount ?? 0} categories`}
          accent={summary?.overBudgetCount > 0 ? 'text-rose-500' : 'text-emerald-500'}
          isLoading={loading}
        />
      </section>

      {/* Budget cards grid - skeleton, empty state, or cards */}
      {loading ? (
        ...
        <>
          ...
          {/* Banner nudging the user to add limits for uncovered categories */}
          {uncovered > 0 && (
            <div className="mt-5 flex items-center gap-3 px-4 py-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-xl animate-slide-down">
              ...
              <button
                onClick={openAdd}
                className="text-xs font-bold text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 underline flex-shrink-0"
              >
                Add limit
              </button>
            </div>
