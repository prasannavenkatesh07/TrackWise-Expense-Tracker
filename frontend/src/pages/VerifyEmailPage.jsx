/**
 * pages/VerifyEmailPage.jsx  (OTP Upgrade - Premium Resend Feature)
 *
 * Flow:
 * 1. User registers → Backend sends 6-digit OTP to email.
 * 2. User is redirected here: /verify-email?email=test@example.com
 * 3. User enters the 6-digit OTP.
 * 4. POST /api/auth/verify-email { email, otp }
 * 5. On success → login() and navigate to Dashboard.
 * ✦ NEW: Resend Code functionality with a 60-second anti-spam cooldown.
 */
...
import {
  Wallet, ArrowRight, Loader2, Mail, AlertCircle, CheckCircle2, RefreshCw
} from 'lucide-react';
...
const VerifyEmailPage = () => {
  ...
  const [countdown, setCountdown]       = useState(0); // Anti-spam timer
    ...
    <main className="relative min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center px-4 py-12 transition-colors duration-300">
      ...
      <div className="relative w-full max-w-md">
        ...
        <article className="card">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                className={`input-field text-center text-3xl tracking-[0.5em] font-mono py-4 ${error ? 'border-rose-400 focus:ring-rose-400' : ''}`}
                placeholder="000000"
                autoFocus
              />
              {error && (
                <p className="flex items-center justify-center gap-1.5 text-xs text-rose-500 mt-3 font-medium" role="alert">
            ...
            <button type="submit" disabled={isSubmitting || otp.length < 6} className="btn-primary w-full py-3">
              {isSubmitting ? <><Loader2 size={17} className="animate-spin" />Verifying…</> : <><CheckCircle2 size={17} />Verify Email</>}
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center space-y-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 text-sm font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isResending ? (
        ...
        <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-6">
          Need to use a different email? <Link to="/register" className="text-emerald-600 hover:underline">Register again</Link>.
        </p>
      </div>
    </main>
  );
